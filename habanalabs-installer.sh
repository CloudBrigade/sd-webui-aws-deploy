...
habanalabs_installer.sh_usage() {
  ...
  if [[ -n $long_usage ]]; then
    ...
    printf "    Default: /var/log/habana_logs/install-$(date -d "today" +"%Y-%m-%d-%H-%M-%S").log\n"
    echo
...
habanalabs_installer.sh_install_usage() {
  ...
  if [[ -n $long_usage ]]; then
    ...
    printf "    Install type (base, dependencies, tensorflow, pytorch)\n"
    printf "    Default: base\n"
    echo
...
HABANA_RELEASE_VERSION="1.12.0"
HABANA_RELEASE_ID="480"
HABANALABS_SCRIPT_LOCK_FILE="/tmp/habanalabs-installer.lock"
REWRITE_CONF="no"
HEADER_VER=$(uname -r)
SUPPORTED_OS=(ubuntu20.04 ubuntu22.04 centos7 amzn2 rhel8.6 debian10.10)
...
HABANALABS_OPTIONAL="habanalabs-firmware-tools habanalabs-qual"
...
BASIC_SCRIPT_DEPENDENCIES="gcc cmake lsof curl wget"
...
HABANALABS_TF_DEPENDENCIES="${BASIC_SCRIPT_DEPENDENCIES} jemalloc mesa-libGL cmake"
...
HABANALABS_TENSORFLOW_VERSION="2.13.0"
TF_IO_VER="0.33.0"
HABANALABS_LIBFABRIC_VERSION="1.18.0"
...
function __print_env(){
    ...
    if [[ "${INSTALL_TYPE}" == "tf" || "${INSTALL_TYPE}" == "tensorflow" ]]; then
        tablerow "TensorFlow version" "$(info "${HABANALABS_TENSORFLOW_VERSION}")"
    fi
...
function __create_log_dir(){
    ...
    if [ ! -d "$HABANA_INSTALLER_LOG_DIR" ]; then
        $_SUDO_CMD mkdir -p "$HABANA_INSTALLER_LOG_DIR" && $_SUDO_CMD chmod -R 777 "$HABANA_INSTALLER_LOG_DIR" || fatal "Failed to create log dir ${HABANA_INSTALLER_LOG_DIR}"
    fi
    $_SUDO_CMD rm -f "${HABANALABS_INSTALLER_LOG}"
}
...
function __install_libfabric(){
    ...
    ./configure --prefix=$HABANALABS_LIBFABRIC_ROOT --enable-debug
    make -j $(nproc --all)
    $_SUDO_CMD make install
    popd
    rm -rf ${_mpi_temp_dir}
...
function update_cache(){
    case $OS_NAME in
        ...
        ;;
        *)
            $_SUDO_CMD "${__pkg__}" makecache 2>/dev/null
        ;;
    esac
}
...
function installPackages(){
    ...
    do
        ...
        __cmd="$_SUDO_CMD ${__pkg__} install ${FORCE_INSTALL} ${OVERWRITE_CONF_FILES} ${package}${version_for_package} 2>&1"
...
function installRepo() {
    ...
    case $OS_NAME in
        "ubuntu"|"debian")
            ...
            curl -s -X GET https://${HABANA_SERVER_NAME}/artifactory/api/gpg/key/public | $_SUDO_CMD apt-key add --
        ;;
    esac
    update_cache
}
...
function __install_python_from_source(){
    ...
    $_SUDO_CMD ./configure --enable-optimizations --enable-loadable-sqlite-extensions --enable-shared
    $_SUDO_CMD make
...
function __install_pt_python_packages(){
    ...
    local __remote_pt_url_dir="https://${HABANA_SERVER_NAME}/${HABANALABS_SERVER_PT_DIR}/${HABANA_RELEASE_VERSION}/${HABANA_RELEASE_ID}/pytorch/${OS_NAME}${__os_version}"
    local __pt_module=""
    if [ -z $HABANALABS_LOCAL_DIR ]; then
...
function __detect_current_pt(){
    if __check_installed_python ; then
        __set_venv_or_skip
        python${MIN_PYTHON_VER} -c 'import sys
try:
    import torch; from habana_frameworks.torch.utils.library_loader import load_habana_module;
except ModuleNotFoundError:
    sys.exit(0)
except:
    sys.exit(1)
else:
    sys.exit(1)' && info "PyTorch package was not detected." || fatal "Please uninstall PyTorch from your system and re-run the script."
    fi
}

# installer TF functions

function __detect_current_tf(){
    if __check_installed_python ; then
        __set_venv_or_skip
        TF_ENABLE_ONEDNN_OPTS=0 TF_CPP_MIN_LOG_LEVEL=2 python${MIN_PYTHON_VER} -c 'import sys
try:
    import tensorflow;
except ModuleNotFoundError:
    sys.exit(0)
    ...
    sys.exit(1)' && info "TensorFlow package was not detected" || fatal "Please uninstall TensorFlow from your system and re-run the script."
    fi
}

function __install_tf_python_packages(){
    local __python_opt=$(__get_python_opt)
    __install_common_python "${__python_opt}" "${HABANALABS_PIP_VERSION}" "${SETUPTOOLS_VERSION}"
    python${MIN_PYTHON_VER} -m pip install tensorflow-cpu==${HABANALABS_TENSORFLOW_VERSION} ${__python_opt} --disable-pip-version-check
    # urllib3 2.x.x installed as dependency tensorflow-cpu, version must be less 2.x on AmazonLinux2 for install and compile habana_horovod
    python${MIN_PYTHON_VER} -m pip install --upgrade "urllib3<2" ${__python_opt} --disable-pip-version-check
    if [ -z $HABANALABS_LOCAL_DIR ]; then
        python${MIN_PYTHON_VER} -m pip install habana_tensorflow=="${HABANA_RELEASE_VERSION}.${HABANA_RELEASE_ID}" ${__python_opt} ${__extra_index_url} --disable-pip-version-check
        python${MIN_PYTHON_VER} -m pip install habana_horovod=="${HABANA_RELEASE_VERSION}.${HABANA_RELEASE_ID}" ${__python_opt} ${__extra_index_url} --disable-pip-version-check
    else
        python${MIN_PYTHON_VER} -m pip install ${HABANALABS_LOCAL_DIR}/habana_tensorflow-${HABANA_RELEASE_VERSION}.${HABANA_RELEASE_ID}-cp*.whl ${__python_opt} --disable-pip-version-check
        python${MIN_PYTHON_VER} -m pip install ${HABANALABS_LOCAL_DIR}/habana-horovod-${HABANA_RELEASE_VERSION}.${HABANA_RELEASE_ID}.tar.gz ${__python_opt} --disable-pip-version-check
    fi
    python${MIN_PYTHON_VER} -m pip install tensorflow-io==${TF_IO_VER} tensorflow-io-gcs-filesystem==${TF_IO_VER} ${__python_opt} --disable-pip-version-check
}
...
function __setup_envs()
{
    if [ $OS_NAME != "ubuntu" ]; then
        ...
        sed -i '/#>>>> TF Habana starts/,/#<<<< TF Habana ends/d' ${HABANALABS_PROFILE_FILE}
        echo "#>>>> TF Habana starts" | tee -a ${HABANALABS_PROFILE_FILE}
        # For AML/CentOS/RHEL OS'es TFIO_DATAPATH have to be specified to import tensorflow_io lib correctly
        echo "export TFIO_DATAPATH=`TF_ENABLE_ONEDNN_OPTS=0 TF_CPP_MIN_LOG_LEVEL=2 python${MIN_PYTHON_VER} -c 'import tensorflow_io as tfio; import os; print(os.path.dirname(os.path.dirname(tfio.__file__)))'`/" | tee -a ${HABANALABS_PROFILE_FILE}
        echo "#<<<< TF Habana ends" | tee -a ${HABANALABS_PROFILE_FILE}
        sed --in-place '/habanalabsrc/d' $HOME/.bashrc
        echo 'source $HOME/.habanalabsrc' | tee -a "${HOME}/.bashrc"
    fi
...
function __validate_tf_packages(){
    __enable_output_log_to_tty
    if [ -z $HABANALABS_DISABLE_VALIDATION ]; then
        ...
        set +e
        TF_ENABLE_ONEDNN_OPTS=0 TF_CPP_MIN_LOG_LEVEL=2 python${MIN_PYTHON_VER} -c 'import tensorflow as tf
...
print("Sanity check passed")'
        if [ $? -ne 0 ]; then
            fatal "Habana TensorFlow test failed."
        else
            info "Habana TensorFlow test was completed successfully"
        fi
        set -e
    else
        info "Skipped Habana TensorFlow test"
    fi
    __disable_output_log_to_tty
}
...
habanalabs_installer.sh_install_command() {
  ...
  # Install frameworks (TF, PT) use user workspace (/home/USERNAME)
  ...
  case $INSTALL_TYPE in
    "tf"|"tensorflow"|"pt"|"pytorch")
      _SUDO_CMD=""
      HABANALABS_INSTALLER_LOG="${HOME}/.habana_logs/install-$(date -d "today" +"%Y-%m-%d-%H-%M-%S").log"
  ...
  case $INSTALL_TYPE in
    "base"|"synapse")
      check_hold_packages
      __install_msg "${_DEPENDENCIES} ${BASIC_SCRIPT_DEPENDENCIES} ${HABANALABS_PRE_REQUIRED} ${HABANALABS_DRIVER} ${HABANALABS_REQUIRED} ${HABANALABS_OPTIONAL}"
    ;;
    "tf"|"tensorflow")
      header "Check TensorFlow packages and dependencies"
      __enable_output_log_to_tty
      __detect_current_cmake
      __info_openmpi
      __detect_installed_python
      __detect_current_tf
      __detect_installed_packages "${HABANALABS_TF_DEPENDENCIES}"
    ...
    "deps"|"dependencies")
      ...
      __info_openmpi
      __detect_python_dependencies
      __detect_installed_packages "$(echo "${BASIC_SCRIPT_DEPENDENCIES} ${HABANALABS_PT_DEPENDENCIES} ${HABANALABS_TF_DEPENDENCIES}" | tr ' ' '\n' | sort | uniq | tr '\n' ' ')"
      __disable_output_log_to_tty
    ;;
    *)
  ...
  if [ -n "${args[--force]}" ]; then
    ...
    read -p "Do you wish to install [y/n]: " -r answer
    answer=${answer:-n}
    if [[ "$answer" == [Yy] ]]; then
      FORCE_INSTALL="-y"
    else
  ...
  case $INSTALL_TYPE in
      "base"|"synapse")
          # install and update repos - must be before any future installation calls
          installRepo
          header "Check device"
          ...
          # uninstall previous version hl-sw if exist
          __prev_sw=$(get_list_current_installed_sw)
          if [ "${__prev_sw}" != "" ]; then
            header "Uninstall previous Habanalabs software"
            uninstall_base_packages
          fi
          header "Install dependencies"
          $_SUDO_CMD $__install_epel__
          installPackages "${_DEPENDENCIES}" "${BASIC_SCRIPT_DEPENDENCIES}"
      ...
      "deps"|"dependencies")
          ...
          update_cache
          installPackages "${BASIC_SCRIPT_DEPENDENCIES} ${HABANALABS_PT_DEPENDENCIES} ${HABANALABS_TF_DEPENDENCIES}"
      ...
      ;;
      "tf"|"tensorflow")
          __install_tf_python_packages
          __setup_envs
          __validate_tf_packages
...
habanalabs_installer.sh_validate_command() {
  ...
  header "TensorFlow dependencies"
  __enable_output_log_to_tty
  __detect_installed_packages "${HABANALABS_TF_DEPENDENCIES}"
...
parse_requirements() {
  ...
  export HABANALABS_INSTALLER_LOG="${HABANALABS_INSTALLER_LOG:-/var/log/habana_logs/install-$(date -d "today" +"%Y-%m-%d-%H-%M-%S").log}"
...
initialize() {
  ...
  case $OS_NAME in
      "ubuntu"|"debian")
          ...
          case $OS_VERSION in
              "20.04"|"10.10")
                  HABANALABS_TF_DEPENDENCIES="libjemalloc2"
              ;;
              "22.04")
                  HABANALABS_TF_DEPENDENCIES="libjemalloc2"
                  MIN_PYTHON_VER="3.10"
          ...
          HABANALABS_TF_DEPENDENCIES="${HABANALABS_TF_DEPENDENCIES} protobuf-compiler libgl1"
      ...
      "amzn")
          ...
          HABANALABS_TF_DEPENDENCIES="${HABANALABS_TF_DEPENDENCIES} gcc-c++ system-lsb-core"
      ...
      "rhel")
          ...
          HABANALABS_TF_DEPENDENCIES="${HABANALABS_TF_DEPENDENCIES} redhat-lsb-core"
